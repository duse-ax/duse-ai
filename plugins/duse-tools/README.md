...
### `navercafe-crawl` — 네이버 카페 전수 크롤링 → 엑셀
네이버 카페 게시판 URL 하나로 **모든 게시글 + 전체 댓글**을 엑셀(정보/게시글/댓글 3시트)로 뽑는다. 목록 API는 비로그인, 본문·댓글은 로그인 필요 — chromux(같은 플러그인)로 로그인하고(캡차는 이미지 판독으로 통과 시도), 댓글 페이지네이션·`[공유]`스크랩글까지 원문 그대로 수집한다. 필요 도구(chromux CLI·Python 라이브러리)는 스킬의 preflight가 자동 설치. **로그인 계정이 대상 카페 회원**이어야 본문이 보인다. 상세·설치 프롬프트 → [skills/navercafe-crawl/](./skills/navercafe-crawl/)

```
"이 네이버 카페 게시판 전부 크롤링해서 엑셀로 만들어줘: https://cafe.naver.com/f-e/cafes/24646884/menus/30"
```
